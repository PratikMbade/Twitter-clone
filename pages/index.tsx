            ...
            <div >
              <ul >
                {sidebarMenuItems.map((item) =>{
                  return <li key={item.id} className='flex font-light  items-center justify-center text-2xl px-3 py-2 pl-2 hover:bg-zinc-800 rounded-full  w-fit cursor-pointer transition-all my-2 '>
                       <span className='text-3xl'>{item.icon}</span>
                       <span className='pl-4 text-xl '>{item.title}</span>
                  </li>
                })}
              </ul>
              <button className='bg-[#1d9bf0] mt-4  rounded-full text-xl font-bold px-24 py-3 hover:bg-[#2488ca] transition-all'>Post</button>
            </div>
          </div>
